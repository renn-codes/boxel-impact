...
public class Tier1Cfg {
    ...
    static {
        BUILDER.push("Config for Tier 1 Banner");
        //Banner name
        BANNER_NAME = BUILDER.comment("Display name of the Tier 1 banner.").define("Banner Name", "Starlight Workshop");
        //Ticket costs
        ENCHANTED_TICKET_COST = BUILDER.comment("Number of Enchanted Tickets required to pull on the Tier 1 banner. Setting the value to 0 disables using Enchanted Tickets for this banner.").define("Ticket Cost", 1);
        NETHER_TICKET_COST = BUILDER.comment("Number of Nether Tickets required to pull on the Tier 1 banner. Setting the value to 0 disables using Nether Tickets for this banner.").define("Ticket Cost", 0);
        ENDER_TICKET_COST = BUILDER.comment("Number of Ender Tickets required to pull on the Tier 1 banner. Setting the value to 0 disables using Ender Tickets for this banner.").define("Ticket Cost", 0);
        //Item config
        COMMON_ITEMS = BUILDER.comment("ItemIDs of 3-star items that can be pulled from the Tier 1 banner, separated by commas.").define("Banner Items", "minecraft:iron_pickaxe,minecraft:iron_axe,minecraft:iron_shovel,minecraft:iron_hoe");
        UNCOMMON_ITEMS = BUILDER.comment("ItemIDs of 4-star items that can be pulled from the Tier 1 banner, separated by commas.").define("Banner Items","minecraft:gold_pickaxe,minecraft:gold_axe,minecraft:gold_shovel,minecraft:gold_hoe");
        RARE_ITEMS = BUILDER.comment("ItemIDs of 5-star items that can be pulled from the Tier 1 banner, separated by commas.").define("Banner Items", "minecraft:diamond_pickaxe,minecraft:diamond_axe,minecraft:diamond_shovel,minecraft:diamond_hoe");
        //Probability config
        ITEMS_PER_PULL = BUILDER.comment("Number of items pulled from the Tier 1 banner per pull. Must be a positive integer.").define("Items per Pull", 1);
        COMMON_CHANCE = BUILDER.comment("Probability of pulling a 3-star item from the Tier 1 banner. Must be a decimal value between 0 and 1.").define("Common Weight", 0.9);
        UNCOMMON_CHANCE = BUILDER.comment("Probability of pulling a 4-star item from the Tier 1 banner. Must be a decimal value between 0 and 1.").define("Uncommon Weight", 0.4);
        RARE_CHANCE = BUILDER.comment("Probability of pulling a 5-star item from the Tier 1 banner. Must be a decimal value between 0 and 1.").define("Rare Weight", 0.1);
